function loadGameObjectLibrary(){
    return new objects();
}
...
class objects {
    ...
    addObject(obj){
        this.objects2D.add(obj);
    }

    removeObject(obj){
        if(this.objects2D.has(obj)){
            this.object2D.delete(obj);
        }
    }
...
class gameObject2D{
    constructor(startX, startY, width, height, isActive, speed, texture = undefined, type = undefined){
        this.type     = type;
        this.x        = startX;
        this.y        = startY;
        this.width    = width;
